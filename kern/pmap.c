...
mem_init(void)
{
	...
	cprintf("kern_pgdir %08x and id %08x\n",  kern_pgdir);
	...
	envs = (struct Env *) boot_alloc(PGSIZE + NENV * sizeof(struct Env));
	//////////////////////////////////////////////////////////////////////
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	//PTE_ADDR is used to remove flag AND PTE_ADDR is physical addrress
	pte_t* tmp = KADDR(PTE_ADDR(*pde_ptr));
	return &tmp[pte_index];
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (pte_ptr == NULL) return -E_NO_MEM;
	else {
		pp -> pp_ref++;
		if (*pte_ptr & PTE_P) page_remove(pgdir, va);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if (base + size >= MMIOLIM) panic("out of MMIOLIM"); 
